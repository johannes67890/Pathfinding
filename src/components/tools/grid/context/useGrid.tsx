...
export const GridContextProvider: React.FC<{ children: ReactNode }> = ({
  children,
}) => {
  const { cellSize } = useContext(CellSizeContext);

  const [grid, setGrid] = useState<Cell[][]>(InitlizeGrid(cellSize));
  const gridCells = useRef<React.RefObject<HTMLButtonElement>[][]>(
    grid.map((row) => row.map(() => React.createRef())),
  );
  const GridValue = useMemo(
    () => ({
      ...
      setGridCells: (newGrid: Cell[][]) => {
        gridCells.current = newGrid.map((row) =>
