...
const RenderButton: React.FC<{ongoing: boolean}> = ({ongoing}) => {
  const { playing, solved } = useContext(ControlContext);
    ...
    if(!playing && !solved && !ongoing)
          {
    ...
    else if (playing && !solved && !ongoing) { // ongoing and playing
            return(
            <div id="status" className="group relative">
              ...
              <button className="button absolute hidden p-0 top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 group-hover:inline ml-auto mr-auto" />
            </div>
    )} else if(!playing && !solved && ongoing) { // paused
      return (
      <div id="status" className="group relative">
                <svg
                  aria-hidden="true"
